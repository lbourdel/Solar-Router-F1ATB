...
void handleAjax_etatActions()
{
  ...
  {
    if ((LesActions[i].Actif > 0) && (ITmode > 0 || i > 0))
    { // Pas de Triac en synchro horloge interne
...
{ // Pins disponibles
  ...
  if (ESP32_Type == 4)
    S = "var Pins=[0,5,18,19,22,23,27,-1];";
  if (ESP32_Type == 10)
...
void handleParaUpdate()
{
  ...
  if (ReacCACSI == 100)
    ReacCACSI = 1;
  if (Vp[39].toInt() == 1 && Source == "Linky")
    ReacCACSI = 100; // EstimCACSI
  int canal = 0;
  for (int c = 0; c < 4; c++)
  {
...
bool Liste_WIFI()
{ // Doit être fait avant toute connection WIFI depuis biblio ESP32 3.0.1
...
void handleAP_SetWifi()
{
  ...
  while (WiFi.status() != WL_CONNECTED && (millis() - newstartMillis < 20000))
...
void ExtraitCookie()
{
  ...
  if (server.hasHeader("Cookie"))
  {
    ...
    cookie.trim();
    Serial.println("cookie.trim():" + cookie);
